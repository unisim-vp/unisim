...
\textit{Note: The UNISIM Virtex 5 FXT Simulator must have the hardware FPU enabled (\texttt{unisim\--virtex5fxt\--1.0\--wfpu}) to run a Linux kernel for \texttt{powerpc\--440fp\--linux\--gnu} or \texttt{powerpc\--440\--linux\--gnu}. When the simulator has the hardware FPU disabled (\texttt{unisim\--virtex5fxt\--1.0}, the simulator can only use the Linux kernel for \texttt{powerpc\--440\--linux\--gnu}.}
