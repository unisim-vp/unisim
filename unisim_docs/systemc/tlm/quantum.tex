...
\begin{frame}
	\frametitle{Temporal decoupling and the Quantum Keeper}
	\begin{block}{Temporal decoupling}
		...
		It allows TLM modles to run ahead of the current simulation time without yielding control to the simulation engine (i.e., without calling \texttt{wait(\ldots)}.
	\end{block}
\end{frame}
