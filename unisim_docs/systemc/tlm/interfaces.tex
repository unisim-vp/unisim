...
However, the TLM OSCI working group recommends to build new modules using TLM2 interfaces and writing rules.
...
\begin{frame}
	...
	\begin{beamercolorbox}[center,rounded=true,wd=\textwidth]{postit}
		Designed for the untimed coding style.
	\end{beamercolorbox}
	\begin{itemize}
		\item<1->Definition:
	\begin{figure}[h]
...
\mode<article>{
The TLM2 blocking interface supports an untimed modeling style.
It defines the \texttt{\textbf{b\_transport}} method, which takes a single non-const reference argument, used by the source module to set the message and by the target module to set the response. 
...
\begin{frame}
	...
	\setbeamercolor{postit}{fg=black,bg=yellow}
	\begin{beamercolorbox}[center,rounded=true,wd=\textwidth]{postit}
		Designed for the loosely- and approximately-timed coding styles.
	\end{beamercolorbox}
	\begin{itemize}
		\item<1->Definition:
	\begin{figure}[h]
	...
	\end{figure}
		\item<2->The \texttt\textbf{TRANS} argument is by default set to \texttt{\textbf{tlm\_generic\_payload}} (recommended).
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Non-blocking transport interface (II)}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item The \texttt{sc\_time} parameter indicates the precise simulation time the call is taking place.
...
\mode<article>{
The non-blocking transport interface exists to support the loosely-timed and approxitemately-timed coding styles.
Figure~\ref{fig:nb_transport_definition} shows the non-blocking transport interface definition.

\begin{figure}[h]
	\lstinputlisting{tlm/nb_transport.cc}
	\caption{The non-blocking transport interface class definition.}
	\label{fig:nb_transport_definition}
\end{figure}

The non-blocking transport interface defines the \texttt{\textbf{nb\_transport}} method which, as the blocking transport interface method \texttt{\textbf{b\_transport}}, passes a non-const reference to the transaction object.
The \texttt{\textbf{nb\_transport}} method also passes two other parameters:
\begin{itemize}
...
\end{itemize}
Additionally the \texttt{\textbf{nb\_transport}} method can return a value (\texttt{\textbf{tlm\_sync\_enum}}) to indicate the status of the call, and signal synchronization points. 

Unlike the blocking transport interface, the non-blocking transport interface needs to be defined by both, the initiator and the target modules.
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|l|l|}
		...
		\hline
		\texttt{TLM\_REJECTED} & Approximately-timed only & Unmodified & No \\
		\hline
...
The debug transaction interface is for debug access free of the delays or side-effects associated with regular transactions.\footnote{For those of you who know UNISIM, this interface is very close (not to say identical) to the non-intrusive memory interface used by the UNISIM simulators.}
