...
\mode<article>{
The non-blocking transport interface exists to support the approxitemately-timed coding style.
...
The non-blocking transport interface defines the \texttt{\textbf{nb\_transport\_fw}} method which, as the blocking transport interface method \texttt{\textbf{b\_transport}}, passes a non-const reference to the transaction object.
The \texttt{\textbf{nb\_transport\_fw}} method also passes two other parameters:
\begin{itemize}
...
\end{itemize}
Additionally the \texttt{\textbf{nb\_transport\_fw}} method can return a value (\texttt{\textbf{tlm\_sync\_enum}}) to indicate the status of the call, and signal synchronization points. 
