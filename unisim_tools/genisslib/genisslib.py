...
class GIL:
    def __init__(self, callname):
        ...
        self.outputprefix = os.path.join(self.prefix, self.output)
        self.verbosity = 1
        self.minwordsize = 32
...
""".format(name=GENISSLIB, version=GENISSLIB_VERSION, date="Friday October 22nd 2021", copyright=COPYRIGHT, authors=AUTHORS, emails=EMAILS)
    ...
    def parse(self, match):
        ...
        elif match( ('--prefix',), '<prefix>', 'Sets the prefix of the generated source files to <prefix> (default is .)' ):
            self.prefix = match.pop_front()
            self.outputprefix = os.path.join(self.prefix, self.output)

        elif match(('-o','--output'), '<output>', 'Sets the path of the generated source files to <output> (default is {!r}).'.format(DEFAULT_OUTPUT) ):
            self.output = match.pop_front()
            self.outputprefix = os.path.join(self.prefix, self.output)
...
class Generator:
    ...
    def iss(self):
        ...
        headerid = '__{}_{}_HH__'.format(tokenize(self.options.output), '__'.join(self.source.namespace))
