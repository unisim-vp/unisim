...
UNISIM_SIMULATORS_DIR=${MY_DIR}/../unisim_simulators/tlm2/avr32emu

AVR32EMU_VERSION=$(cat ${UNISIM_SIMULATORS_DIR}/VERSION)
GENISSLIB_VERSION=$(cat ${UNISIM_TOOLS_DIR}/genisslib/VERSION)-avr32emu-${AVR32EMU_VERSION}
...
UNISIM_LIB_AVR32EMU_SOURCE_FILES="\
...
UNISIM_LIB_AVR32EMU_HEADER_FILES="${UNISIM_LIB_AVR32EMU_ISA_FILES} \
...
UNISIM_LIB_AVR32EMU_TEMPLATE_FILES="\
...
UNISIM_SIMULATORS_AVR32EMU_SOURCE_FILES="\
config.cc \
memory_router.cc \
...
UNISIM_SIMULATORS_AVR32EMU_TEMPLATE_FILES="\
simulator.tcc \
"

UNISIM_SIMULATORS_AVR32EMU_DATA_FILES="\
COPYING \
INSTALL \
NEWS \
README \
...
UNISIM_SIMULATORS_AVR32EMU_TESTBENCH_FILES="\
main.cc \
main_debug.cc \
"
...
has_to_build_avr32emu_configure=no

mkdir -p ${DEST_DIR}/genisslib
mkdir -p ${DEST_DIR}/avr32emu
...
UNISIM_LIB_AVR32EMU_FILES="${UNISIM_LIB_AVR32EMU_SOURCE_FILES} ${UNISIM_LIB_AVR32EMU_HEADER_FILES} ${UNISIM_LIB_AVR32EMU_TEMPLATE_FILES} ${UNISIM_LIB_AVR32EMU_DATA_FILES}"

for file in ${UNISIM_LIB_AVR32EMU_FILES}; do
	mkdir -p "${DEST_DIR}/avr32emu/$(dirname ${file})"
	has_to_copy=no
	if [ -e "${DEST_DIR}/avr32emu/${file}" ]; then
		if [ "${UNISIM_LIB_DIR}/${file}" -nt "${DEST_DIR}/avr32emu/${file}" ]; then
			has_to_copy=yes
		fi
	else
		has_to_copy=yes
	...
	if [ "${has_to_copy}" = "yes" ]; then
		echo "${UNISIM_LIB_DIR}/${file} ==> ${DEST_DIR}/avr32emu/${file}"
		${DISTCOPY} -f "${UNISIM_LIB_DIR}/${file}" "${DEST_DIR}/avr32emu/${file}" || exit
	fi
done

UNISIM_SIMULATORS_AVR32EMU_FILES="${UNISIM_SIMULATORS_AVR32EMU_SOURCE_FILES} ${UNISIM_SIMULATORS_AVR32EMU_HEADER_FILES} ${UNISIM_SIMULATORS_AVR32EMU_TEMPLATE_FILES} ${UNISIM_SIMULATORS_AVR32EMU_DATA_FILES} ${UNISIM_SIMULATORS_AVR32EMU_TESTBENCH_FILES}"

for file in ${UNISIM_SIMULATORS_AVR32EMU_FILES}; do
	has_to_copy=no
	if [ -e "${DEST_DIR}/avr32emu/${file}" ]; then
		if [ "${UNISIM_SIMULATORS_DIR}/${file}" -nt "${DEST_DIR}/avr32emu/${file}" ]; then
			has_to_copy=yes
		fi
	else
		has_to_copy=yes
	...
	if [ "${has_to_copy}" = "yes" ]; then
		echo "${UNISIM_SIMULATORS_DIR}/${file} ==> ${DEST_DIR}/avr32emu/${file}"
		${DISTCOPY} -f "${UNISIM_SIMULATORS_DIR}/${file}" "${DEST_DIR}/avr32emu/${file}" || exit
	fi
done

for file in ${UNISIM_SIMULATORS_AVR32EMU_DATA_FILES}; do
	has_to_copy=no
...
mkdir -p ${DEST_DIR}/avr32emu/config
mkdir -p ${DEST_DIR}/avr32emu/m4
...
for file in ${UNISIM_LIB_AVR32EMU_M4_FILES}; do
	has_to_copy=no
	if [ -e "${DEST_DIR}/avr32emu/${file}" ]; then
		if [ "${UNISIM_LIB_DIR}/${file}" -nt  "${DEST_DIR}/avr32emu/${file}" ]; then
			has_to_copy=yes
	...
	if [ "${has_to_copy}" = "yes" ]; then
		echo "${UNISIM_LIB_DIR}/${file} ==> ${DEST_DIR}/avr32emu/${file}"
		${DISTCOPY} -f "${UNISIM_LIB_DIR}/${file}" "${DEST_DIR}/avr32emu/${file}" || exit
		has_to_build_avr32emu_configure=yes
	fi
done
...
if [ "${has_to_build_configure}" = "yes" ]; then
	...
	echo "AC_INIT([UNISIM AVR32EMU Simulator Package], [${AVR32EMU_VERSION}], [Julien Lisita <julien.lisita@cea.fr>, Gilles Mouchard <gilles.mouchard@cea.fr>], [unisim-avr32emu])" > "${DEST_DIR}/configure.ac"
	...
	echo "AC_CONFIG_SUBDIRS([avr32emu])"  >> "${CONFIGURE_AC}" 
	echo "AC_CONFIG_FILES([Makefile])" >> "${CONFIGURE_AC}"
	echo "AC_OUTPUT" >> "${CONFIGURE_AC}"

	echo "Generating Makefile.am"
	echo "SUBDIRS=genisslib avr32emu" > "${MAKEFILE_AM}"
...
if test "\${help}" = "yes"; then
	echo "=== configure help for avr32emu"
else
	echo "=== configuring in avr32emu (\${HERE}/avr32emu) for \${host} host system type"
	echo "\$(basename \$0): running \${MY_DIR}/avr32emu/configure \$@"
fi

if test ! -d \${HERE}/avr32emu; then
	mkdir \${HERE}/avr32emu
fi
cd \${HERE}/avr32emu
\${MY_DIR}/avr32emu/configure "\$@"
STATUS="\$?"
cd "\${HERE}"
if test \${STATUS} -ne 0; then
	exit \${STATUS}
...
#!/usr/bin/make -f
all: avr32emu-all
clean: genisslib-clean avr32emu-clean
distclean: genisslib-distclean avr32emu-distclean
	rm -f \${HERE}/Makefile.cross
install: avr32emu-install

genisslib-all:
	@\\\$(MAKE) -C \${HERE}/genisslib all
avr32emu-all: genisslib-all
	@\\\$(MAKE) -C \${HERE}/avr32emu all
genisslib-clean:
	@\\\$(MAKE) -C \${HERE}/genisslib clean
avr32emu-clean:
	@\\\$(MAKE) -C \${HERE}/avr32emu clean
genisslib-distclean:
	@\\\$(MAKE) -C \${HERE}/genisslib distclean
avr32emu-distclean:
	@\\\$(MAKE) -C \${HERE}/avr32emu distclean
avr32emu-install:
	@\\\$(MAKE) -C \${HERE}/avr32emu install
EOF_MAKEFILE_CROSS
...
AVR32EMU_CONFIGURE_AC="${DEST_DIR}/avr32emu/configure.ac"
AVR32EMU_MAKEFILE_AM="${DEST_DIR}/avr32emu/Makefile.am"


if [ ! -e "${AVR32EMU_CONFIGURE_AC}" ]; then
	has_to_build_avr32emu_configure=yes
else
	if [ "$0" -nt "${AVR32EMU_CONFIGURE_AC}" ]; then
		has_to_build_avr32emu_configure=yes
	fi
fi

if [ ! -e "${AVR32EMU_MAKEFILE_AM}" ]; then
	has_to_build_avr32emu_configure=yes
else
	if [ "$0" -nt "${AVR32EMU_MAKEFILE_AM}" ]; then
		has_to_build_avr32emu_configure=yes
	fi
fi

if [ "${has_to_build_avr32emu_configure}" = "yes" ]; then
	echo "Generating avr32emu configure.ac"
	echo "AC_INIT([UNISIM AVR32EMU Standalone simulator], [${AVR32EMU_VERSION}], [Julien Lisita <julien.lisita@cea.fr>, Gilles Mouchard <gilles.mouchard@cea.fr>], [unisim-avr32emu-core])" > "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_CONFIG_MACRO_DIR([m4])" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_CONFIG_AUX_DIR(config)" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_CONFIG_HEADERS([config.h])" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_CANONICAL_BUILD" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_CANONICAL_HOST" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_CANONICAL_TARGET" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AM_INIT_AUTOMAKE([subdir-objects tar-pax])" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_PATH_PROGS(SH, sh)" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_PROG_CXX" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_PROG_RANLIB" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_PROG_INSTALL" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_PROG_LN_S" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_LANG([C++])" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AM_PROG_CC_C_O" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "CPPFLAGS=\"${CPPFLAGS} -D_LARGEFILE64_SOURCE\"" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_CHECK_HEADERS([${AVR32EMU_EXTERNAL_HEADERS}],, AC_MSG_ERROR([Some external headers are missing.]))" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "case \"\${host}\" in" >> "${AVR32EMU_CONFIGURE_AC}"
	printf "\t*mingw*)\n" >> "${AVR32EMU_CONFIGURE_AC}"
	printf "\t;;\n" >> "${AVR32EMU_CONFIGURE_AC}"
	printf "\t*)\n" >> "${AVR32EMU_CONFIGURE_AC}"
	printf "\tUNISIM_CHECK_PTHREAD(main)\n" >> "${AVR32EMU_CONFIGURE_AC}"
	printf "\t;;\n" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "esac" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "UNISIM_CHECK_TIMES(main)" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "UNISIM_CHECK_ENDIAN(main)" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "UNISIM_CHECK_CURSES(main)" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "UNISIM_CHECK_LIBEDIT(main)" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "UNISIM_CHECK_BSD_SOCKETS(main)" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "UNISIM_CHECK_ZLIB(main)" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "UNISIM_CHECK_LIBXML2(main)" >> "${AVR32EMU_CONFIGURE_AC}"
	...
	echo "AC_CONFIG_FILES([Makefile])" >> "${AVR32EMU_CONFIGURE_AC}"
	echo "AC_OUTPUT" >> "${AVR32EMU_CONFIGURE_AC}"
	...
	echo "bin_PROGRAMS = unisim-avr32emu-${AVR32EMU_VERSION} unisim-avr32emu-debug-${AVR32EMU_VERSION}" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "unisim_avr32emu_${AM_AVR32EMU_VERSION}_SOURCES = main.cc ${UNISIM_SIMULATORS_AVR32EMU_SOURCE_FILES}" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "unisim_avr32emu_${AM_AVR32EMU_VERSION}_LDADD = libavr32emu-${AVR32EMU_VERSION}.a" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "unisim_avr32emu_debug_${AM_AVR32EMU_VERSION}_SOURCES = main_debug.cc ${UNISIM_SIMULATORS_AVR32EMU_SOURCE_FILES}" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "unisim_avr32emu_debug_${AM_AVR32EMU_VERSION}_LDADD = libavr32emu-${AVR32EMU_VERSION}.a" >> "${AVR32EMU_MAKEFILE_AM}"

	echo "noinst_HEADERS = ${UNISIM_LIB_AVR32EMU_HEADER_FILES} ${UNISIM_LIB_AVR32EMU_TEMPLATE_FILES} ${UNISIM_SIMULATORS_AVR32EMU_HEADER_FILES} ${UNISIM_SIMULATORS_AVR32EMU_TEMPLATE_FILES}" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "EXTRA_DIST = ${UNISIM_LIB_AVR32EMU_M4_FILES}" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "sharedir = \$(prefix)/share/unisim-avr32emu-${AVR32EMU_VERSION}" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "dist_share_DATA = ${UNISIM_LIB_AVR32EMU_DATA_FILES} ${UNISIM_SIMULATORS_AVR32EMU_DATA_FILES}" >> "${AVR32EMU_MAKEFILE_AM}"

	echo "BUILT_SOURCES=\$(top_builddir)/unisim/component/cxx/processor/avr32/avr32a/avr32uc/isa.hh \$(top_builddir)/unisim/component/cxx/processor/avr32/avr32a/avr32uc/isa.tcc" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "CLEANFILES=\$(top_builddir)/unisim/component/cxx/processor/avr32/avr32a/avr32uc/isa.hh \$(top_builddir)/unisim/component/cxx/processor/avr32/avr32a/avr32uc/isa.tcc" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "\$(top_builddir)/unisim/component/cxx/processor/avr32/avr32a/avr32uc/isa.tcc: \$(top_builddir)/unisim/component/cxx/processor/avr32/avr32a/avr32uc/isa.hh" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "\$(top_builddir)/unisim/component/cxx/processor/avr32/avr32a/avr32uc/isa.hh: ${UNISIM_LIB_AVR32EMU_ISA_FILES}" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\t" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "\$(GENISSLIB_PATH) -o \$(top_builddir)/unisim/component/cxx/processor/avr32/avr32a/avr32uc/isa -w 8 -I \$(top_srcdir) \$(top_srcdir)/unisim/component/cxx/processor/avr32/avr32a/avr32uc/isa/avr32uc.isa" >> "${AVR32EMU_MAKEFILE_AM}"

	echo "all-local: all-local-bin all-local-share" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "clean-local: clean-local-bin clean-local-share" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "all-local-bin: \$(bin_PROGRAMS)" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\t@PROGRAMS='\$(bin_PROGRAMS)'; \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	...
	printf "\tdone\n" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "clean-local-bin:" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\t@if [ ! -z '\$(bin_PROGRAMS)' ]; then \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\trm -rf '\$(top_builddir)/bin'; \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\tfi\n" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "all-local-share: \$(dist_share_DATA)" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\t@SHARED_DATAS='\$(dist_share_DATA)'; \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\tfor SHARED_DATA in \$\${SHARED_DATAS}; do \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\trm -f \"\$(top_builddir)/share/unisim-avr32emu-${AVR32EMU_VERSION}/\$\$(basename \$\${SHARED_DATA})\"; \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\tmkdir -p '\$(top_builddir)/share/unisim-avr32emu-${AVR32EMU_VERSION}'; \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\tcp -f \"\$(top_srcdir)/\$\${SHARED_DATA}\" \$(top_builddir)/share/unisim-avr32emu-${AVR32EMU_VERSION}/\$\$(basename \"\$\${SHARED_DATA}\"); \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\tdone\n" >> "${AVR32EMU_MAKEFILE_AM}"
	echo "clean-local-share:" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\t@if [ ! -z '\$(dist_share_DATA)' ]; then \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\trm -rf '\$(top_builddir)/share'; \\\\\n" >> "${AVR32EMU_MAKEFILE_AM}"
	printf "\tfi\n" >> "${AVR32EMU_MAKEFILE_AM}"

	echo "Building avr32emu configure"
	${SHELL} -c "cd ${DEST_DIR}/avr32emu && aclocal -I m4 && autoconf --force && autoheader && automake -ac"
fi
