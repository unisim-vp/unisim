...
if [ "${has_to_build_configure}" = "yes" ]; then
	...
	echo "AC_INIT([UNISIM PowerMAC G4 PCI Standalone simulator], [${PPCEMU_VERSION}], [Gilles Mouchard <gilles.mouchard@cea.fr>, Daniel Gracia Perez <daniel.gracia-perez@cea.fr>, Reda Nouacer <reda.nouacer@cea.fr>], [ppcemu])" > "${DEST_DIR}/configure.ac"
...
if [ "${has_to_build_ppcemu_configure}" = "yes" ]; then
	...
	echo "AC_INIT([UNISIM PowerMac G4 PCI C++ simulator], [${PPCEMU_VERSION}], [Gilles Mouchard <gilles.mouchard@cea.fr>, Daniel Gracia Perez <daniel.gracia-perez@cea.fr>, Reda Nouacer <reda.nouacer@cea.fr>], [ppcemu-core])" > "${PPCEMU_CONFIGURE_AC}"
	...
	echo "AC_DEFINE([BIN_TO_SHARED_DATA_PATH], [\"../share/ppcemu-${PPCEMU_VERSION}\"], [path of shared data relative to bin directory])" >> "${PPCEMU_CONFIGURE_AC}"
	...
	echo "bin_PROGRAMS = ppcemu-${PPCEMU_VERSION} ppcemu-debug-${PPCEMU_VERSION}" >> "${PPCEMU_MAKEFILE_AM}"
	echo "ppcemu_${AM_PPCEMU_VERSION}_SOURCES = ${UNISIM_LIB_PPCEMU_SOURCE_FILES} ${UNISIM_SIMULATORS_PPCEMU_SOURCE_FILES}" >> "${PPCEMU_MAKEFILE_AM}"
	echo "ppcemu_debug_${AM_PPCEMU_VERSION}_SOURCES = ${UNISIM_LIB_PPCEMU_SOURCE_FILES} ${UNISIM_SIMULATORS_PPCEMU_SOURCE_FILES}" >> "${PPCEMU_MAKEFILE_AM}"
	echo "ppcemu_debug_${AM_PPCEMU_VERSION}_CPPFLAGS = -DDEBUG_PPCEMU" >> "${PPCEMU_MAKEFILE_AM}"
	echo "noinst_HEADERS = ${UNISIM_TOOLS_PPCEMU_HEADER_FILES} ${UNISIM_LIB_PPCEMU_HEADER_FILES} ${UNISIM_LIB_PPCEMU_TEMPLATE_FILES} ${UNISIM_SIMULATORS_PPCEMU_HEADER_FILES} ${UNISIM_SIMULATORS_PPCEMU_TEMPLATE_FILES}" >> "${PPCEMU_MAKEFILE_AM}"
	echo "EXTRA_DIST = ${UNISIM_LIB_PPCEMU_M4_FILES}" >> "${PPCEMU_MAKEFILE_AM}"
	echo "sharedir = \$(prefix)/share/ppcemu-${PPCEMU_VERSION}" >> "${PPCEMU_MAKEFILE_AM}"
	...
	printf "\trm -f \"\$(top_builddir)/share/ppcemu-${PPCEMU_VERSION}/\`basename \$\${SHARED_DATA}\`\"; \\\\\n" >> "${PPCEMU_MAKEFILE_AM}"
	printf "\tmkdir -p '\$(top_builddir)/share/ppcemu-${PPCEMU_VERSION}'; \\\\\n" >> "${PPCEMU_MAKEFILE_AM}"
	printf "\t(cd '\$(top_builddir)/share/ppcemu-${PPCEMU_VERSION}' && cp -f \"\$(abs_top_builddir)/\$\${SHARED_DATA}\" \`basename \"\$\${SHARED_DATA}\"\`); \\\\\n" >> "${PPCEMU_MAKEFILE_AM}"
	printf "\tdone\n" >> "${PPCEMU_MAKEFILE_AM}"
