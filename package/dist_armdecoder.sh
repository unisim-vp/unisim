...
AC_INIT([UNISIM ARMDecoder ARMv7 decoder plugin for formal debugger], [${SIMULATOR_VERSION}], [Yves Lhuillier <yves.lhuillier@cea.fr>], [unisim-${SIMPKG}-core])
...
unisim_${AM_SIMPKG}_${AM_SIMULATOR_VERSION}_SOURCES = ${UNISIM_TESTMAIN_SOURCE_FILES}
...
libunisim_${AM_SIMPKG}_${AM_SIMULATOR_VERSION}_la_SOURCES = ${UNISIM_LIB_SIMULATOR_SOURCE_FILES} ${UNISIM_SIMULATOR_SOURCE_FILES}
...
\$(top_builddir)/top_arm32.hh: ${UNISIM_SIMULATOR_TOP_ARM32_ISA} ${UNISIM_LIB_SIMULATOR_ISA_ARM32_FILES}
	\$(PYTHON_BIN) \$(top_srcdir)/genisslib.py -o \$(top_builddir)/top_arm32 -w 8 -I \$(top_srcdir) \$(top_srcdir)/${UNISIM_SIMULATOR_TOP_ARM32_ISA}
...
\$(top_builddir)/top_thumb.hh: ${UNISIM_SIMULATOR_TOP_THUMB_ISA} ${UNISIM_LIB_SIMULATOR_ISA_THUMB_FILES}
	\$(PYTHON_BIN) \$(top_srcdir)/genisslib.py -o \$(top_builddir)/top_thumb -w 8 -I \$(top_srcdir) \$(top_srcdir)/${UNISIM_SIMULATOR_TOP_THUMB_ISA}
