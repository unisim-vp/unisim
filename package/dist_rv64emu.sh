#!/bin/bash
SIMPKG=rv64emu
SIMPKG_SRCDIR=cxx/rv64emu
SIMPKG_DSTDIR=rv64emu
...
This package contains:
  - RiscV64Emu: an little endian Riscv simulator
...
AC_INIT([UNISIM RiscV64Emu little endian RiscV rv64 simulator package], [${SIMULATOR_VERSION}], [Yves Lhuillier <yves.lhuillier@cea.fr>], [unisim-${SIMPKG}])
...
AC_INIT([UNISIM RiscV64Emu Risc-V 64-bits simulator], [${SIMULATOR_VERSION}], [Yves Lhuillier <yves.lhuillier@cea.fr>], [unisim-${SIMPKG}-core])
...
\$(top_builddir)/${UNISIM_ISA_PREFIX}_rv64.hh: ${UNISIM_LIB_SIMULATOR_ISA_FILES}
	\$(GENISSLIB_PATH) -o \$(top_builddir)/${UNISIM_ISA_PREFIX}_rv64 -w 8 -I \$(top_srcdir) \$(top_srcdir)/${UNISIM_ISA_PREFIX}/rv64/riscv.isa
