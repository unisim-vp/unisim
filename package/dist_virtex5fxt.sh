...
for file in ${UNISIM_LIB_VIRTEX5FXT_FILES}; do
	...
	if [ "${has_to_copy}" = "yes" ]; then
		...
		cp -f "${UNISIM_LIB_DIR}/${file}" "${DEST_DIR}/virtex5fxt/${file}" || exit
	fi
done
...
for file in ${UNISIM_SIMULATORS_VIRTEX5FXT_FILES}; do
	...
	if [ "${has_to_copy}" = "yes" ]; then
		...
		cp -f "${UNISIM_SIMULATORS_DIR}/${file}" "${DEST_DIR}/virtex5fxt/${file}" || exit
	fi
done
...
if [ "${has_to_build_virtex5fxt_configure}" = "yes" ]; then
	...
	echo "UNISIM_CHECK_SYSTEMC(sc_main)" >> "${VIRTEX5FXT_CONFIGURE_AC}"
	echo "UNISIM_CHECK_TLM20(sc_main)" >> "${VIRTEX5FXT_CONFIGURE_AC}"
