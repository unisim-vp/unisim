...
if [ "${has_to_build_armemu_configure}" = "yes" ]; then
	...
	echo "INCLUDES=-I\$(top_srcdir) -I\$(top_builddir)" >> "${ARMEMU_MAKEFILE_AM}"
	...
	printf "\t(cd '\$(top_builddir)/bin' && cp -f \"\$(top_builddir)/\$\${PROGRAM}\" \`basename \"\$\${PROGRAM}\"\`); \\\\\n" >> "${ARMEMU_MAKEFILE_AM}"
	printf "\tdone\n" >> "${ARMEMU_MAKEFILE_AM}"
	...
	printf "\t(cd '\$(top_builddir)/share/unisim-armemu-${ARMEMU_VERSION}' && cp -f \"\$(top_srcdir)/\$\${SHARED_DATA}\" \`basename \"\$\${SHARED_DATA}\"\`); \\\\\n" >> "${ARMEMU_MAKEFILE_AM}"
	printf "\tdone\n" >> "${ARMEMU_MAKEFILE_AM}"
