        ...
        if (cpsr.Get( unisim::component::cxx::processor::arm::T ))
          {
            ...
            ArmBranch::PrintNextPC( std::cerr, *this, insn );
...
namespace {
  template <class T> struct ABD {};
  
  /** Decoder for the ARM32 instruction set. */
  template <> struct ABD<unisim::component::cxx::processor::arm::isa::arm32::CodeType>
    : public unisim::component::cxx::processor::arm::isa::arm32::Decoder<ArmBranch>
  {
    enum { thumb = 0 };
  };
  
  /** Decoder for the THUMB instruction set. */
  template <> struct ABD<unisim::component::cxx::processor::arm::isa::thumb::CodeType>
    : public unisim::component::cxx::processor::arm::isa::thumb::Decoder<ArmBranch>
  {
    enum { thumb = 1 };
  };
  
  template <class CT>
  void Do( std::ostream& sink, ArmProcessor& ap, ABD<CT>& abd, CT const& insn )
  {
    uint32_t insn_addr = ap.GetCIA();
    auto operation = abd.NCDecode( insn_addr, insn );

    ActionNode root;
    for (bool end = false; not end;)
      {
        ArmBranch ab( root, insn_addr, ABD<CT>::thumb );
        operation->execute( ab );
        if (ab.next_insn_addr.determined)
          sink << "NPC: " << ab.next_insn_addr.value << std::endl;
        end = ab.path->close();
      }
...
void
ArmBranch::PrintNextPC( std::ostream& sink, ArmProcessor& pr,
                        unisim::component::cxx::processor::arm::isa::thumb::CodeType const& insn )
{
  static ABD<unisim::component::cxx::processor::arm::isa::thumb::CodeType> abd;
  Do( sink, pr, abd, insn );
}
...
{
  static ABD<unisim::component::cxx::processor::arm::isa::arm32::CodeType> abd;
  Do( sink, pr, abd, insn );
}
